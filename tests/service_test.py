...
def test_get_config(client):
    response = client.simulate_get("/configs")
...
def test_get_help(client):
    ...
    response = client.simulate_get("/help")
    assert response.status == "200 OK"
    assert response.text == Help.HELP_TEXT


def test_usage_no_access_id(client):
    response = client.simulate_get("/usage/email")
    ...
    response = client.simulate_post("/usage/email")
    assert response.status == "401 Unauthorized"
    assert response.json == {
        "error": "Unauthorized",
        "message": "Access-ID header is missing",
...
def test_email_limit(client):
    response = client.simulate_get("/usage/email", headers={"Access-ID": "ananto"})
    assert response.status == "200 OK"
    assert response.json == {"access_in_ms": 0}

    response = client.simulate_post("/usage/email", headers={"Access-ID": "ananto"})
